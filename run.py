...
def load_third_party_model(config: Config) -> BaseDeepakeDetectionModel:
    ...
    if "weights/BiasConsist" in config.checkpoint or "bias_consistency" in config.checkpoint:
        from src.model.BiasConsistency import BiasConsistencyDetector

        return BiasConsistencyDetector.load_from_checkpoint(config.checkpoint)

    if "GenD" in config.checkpoint or "yermandy/" in config.checkpoint:
